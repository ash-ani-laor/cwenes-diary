...
export const formatWaerDateTime = (luxonDate) => {
  if (!luxonDate || !luxonDate.isValid) {
    return {
      ...
      waerDayOfYear: "00",
      waerTime: "00:00:00",
      toString: () => "00-00-0000 00:00:00",
    };
  }
  const waerDate = convertToWaerDate(luxonDate);
  const waerTime = convertToWaerTime(luxonDate);
  const xoggRune = new XoggRune();
  const waerDayStr = xoggRune.Decimal2XoggvaNumber(waerDate.waerDay, 2);
  const waerYearStr = xoggRune.Decimal2XoggvaNumber(waerDate.waerYear, 2);
  const waerMonthStr = xoggRune.ChangeTextEncoding(
    waerMonths[waerDate.waerMonth - 1].toLowerCase(),
    2,
    1
  );
  const waerDayOfYear = getWaerDayOfYear(luxonDate);
  const waerHourStr = xoggRune.Decimal2XoggvaNumber(waerTime.waerHour, 2);
  const waerMinuteStr = xoggRune.Decimal2XoggvaNumber(waerTime.waerMinute, 2);
  const waerSecondStr = xoggRune.Decimal2XoggvaNumber(waerTime.waerSecond, 2);
  const result = {
    ...
    waerDayOfYear,
    waerTime: `${waerHourStr}:${waerMinuteStr}:${waerSecondStr}`,
    toString: () =>
      `${waerDayStr}/${waerMonthStr}/${waerYearStr} ${waerHourStr}:${waerMinuteStr}:${waerSecondStr}`,
  };
  return result;
};
